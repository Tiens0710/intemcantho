...
const fileRequirements = [
  "File dùng kích thước: A5 (14.8x21cm), A4 (21x29.7cm) hoặc A3 (29.7x42cm)",
  "Độ phân giải 150-300dpi, hệ màu CMYK",
...
const testimonials = [
  {
    id: 1,
    content:
      "Tờ rơi in rất sắc nét, màu đẹp, nhân viên tư vấn nhiệt tình. Giao đúng hẹn, sẽ ủng hộ lâu dài!",
    author: "Chị Hạnh",
    role: "Chủ spa Linh Nhi",
    avatar:
      "https://ui-avatars.com/api/?name=Hanh&background=fcebdc&color=e6792a",
    rating: 5,
  },
  {
    id: 2,
    content:
      "Giá hợp lý, chất lượng tốt. Dùng tờ rơi phát cho khách hàng rất tiện lợi.",
    author: "Anh Khoa",
    role: "Marketing Manager",
    avatar:
      "https://ui-avatars.com/api/?name=Khoa&background=fcebdc&color=e6792a",
    rating: 5,
  },
  {
    id: 3,
    content:
      "In nhanh, hỗ trợ nhiệt tình, file chưa chuẩn cũng được hướng dẫn chỉnh lại rất kỹ.",
    author: "Chị Mai",
    role: "Chủ shop mỹ phẩm",
    avatar:
      "https://ui-avatars.com/api/?name=Mai&background=fcebdc&color=e6792a",
    rating: 5,
  },
];
...
export default function ToRoiFileAndFeedback() {
  ...
  return (
    <section ref={sectionRef} className="py-16 lg:py-20 bg-white relative overflow-hidden">
      ...
      <div className="pointer-events-none absolute -bottom-32 -left-32 w-[400px] h-[400px] rounded-full opacity-[0.03]" style={{ background: "radial-gradient(circle, #E6792A 0%, transparent 70%)" }} />
            ...
            <div className="relative bg-white rounded-2xl p-6 lg:p-8 h-full overflow-hidden" style={{ border: "1.5px solid #fcebdc", boxShadow: "0 4px 24px rgba(230,121,42,0.06), 0 1px 4px rgba(230,121,42,0.04)" }}>
              ...
              <div className="absolute top-4 right-4 opacity-[0.06]">
                <svg width="64" height="64" viewBox="0 0 24 24" fill="none">
                  ...
                  <polyline points="14 2 14 8 20 8" fill="none" stroke="#E6792A" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
              </div>
              ...
              <motion.div initial={{ opacity: 0, y: -12 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.2 }} className="text-center mb-7">
                <h2 className="text-xl font-black text-center text-gray-900 uppercase tracking-tight" style={{ fontFamily: "'Nunito', sans-serif" }}>
                  YÊU CẦU FILE{" "}
                  <span style={{ color: "#E6792A" }}>IN TỜ RƠI</span>
                </h2>
              ...
              <motion.ul variants={containerVariants} initial="hidden" animate={isInView ? "visible" : "hidden"} className="space-y-4 mb-8">
                {fileRequirements.map((req, idx) => (
                  <motion.li key={idx} variants={itemVariants} className="flex items-start gap-3 group/item">
                    ...
                    <span className="text-[14px] text-gray-800 font-bold leading-snug transition-colors duration-200 group-hover/item:text-[#E6792A]" style={{ fontFamily: "'Nunito', sans-serif" }}>
                      {req}
                    </span>
                  </motion.li>
                ))}
...
function TestimonialSwiper({ isInView }: { isInView: boolean }) {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 20 }} animate={isInView ? { opacity: 1, y: 0 } : {}} transition={{ duration: 0.5, delay: 0.3 }}>
      <div ref={scrollRef} className="flex gap-4 overflow-x-auto snap-x snap-mandatory pb-2" style={{ scrollbarWidth: "none", msOverflowStyle: "none", WebkitOverflowScrolling: "touch" }}>
        <style>{`.testimonial-scroll::-webkit-scrollbar { display: none; }`}</style>
        {DOUBLED.map((testi, idx) => (
          <TestimonialCard key={`${testi.id}-${idx}`} testimonial={testi} />
        ))}
      </div>

      <div className="flex justify-center gap-2.5 mt-6">
        {testimonials.map((_, idx) => (
          <button key={idx} onClick={() => goToPage(idx)} className="relative transition-all duration-300 cursor-pointer rounded-full" style={{ width: idx === activePage ? "28px" : "10px", height: "10px", background: idx === activePage ? "#E6792A" : "#e5e7eb", boxShadow: idx === activePage ? "0 0 0 3px rgba(230,121,42,0.15), 0 2px 4px rgba(230,121,42,0.25)" : "none" }} aria-label={`Go to testimonial ${idx + 1}`} />
        ))}
      </div>
    </motion.div>
  );
...
function TestimonialCard({ testimonial }: { testimonial: (typeof testimonials)[number] }) {
  return (
    <motion.div variants={cardVariants} initial="hidden" whileInView="visible" viewport={{ once: true }} className="group relative flex-shrink-0 w-[calc(100%-8px)] md:w-[calc(50%-8px)] lg:w-[calc(33.333%-11px)] snap-start">
      <motion.div whileHover={{ y: -6 }} transition={{ type: "spring", stiffness: 300, damping: 20 }} className="relative bg-white rounded-2xl p-5 flex flex-col h-full overflow-hidden" style={{ border: "1.5px solid #f4f4f5", boxShadow: "0 2px 12px rgba(0,0,0,0.04), 0 1px 3px rgba(0,0,0,0.03)" }}>
        <div className="absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-500 group-hover:opacity-100" style={{ background: "radial-gradient(ellipse at center, rgba(230,121,42,0.05) 0%, transparent 70%)" }} />
        ...
        <div className="absolute inset-0 rounded-2xl opacity-0 transition-opacity duration-300 group-hover:opacity-100" style={{ border: "1.5px solid #fcebdc" }} />

        <div className="relative z-10 flex flex-col h-full">
          <div className="flex justify-center gap-1 mb-4">
            {Array.from({ length: 5 }).map((_, i) => (
              <svg key={i} width="16" height="16" viewBox="0 0 24 24" fill={i < testimonial.rating ? "#F5A623" : "#e5e7eb"}>
                <path d="M12 2L15.09 8.26L22 9.27L17 14.14L18.18 21.02L12 17.77L5.82 21.02L7 14.14L2 9.27L8.91 8.26L12 2Z" />
              </svg>
            ))}
          </div>
          <div className="flex justify-center mb-3">
            <svg width="28" height="28" viewBox="0 0 24 24" fill="none" className="opacity-10 group-hover:opacity-20 transition-opacity duration-300">
              <path d="M3 21C3 21 4 15 8 12C8 12 5 11 3 13V21ZM12 21C12 21 13 15 17 12C17 12 14 11 12 13V21Z" fill="#E6792A" />
            </svg>
          </div>
          <p className="text-[13px] text-gray-700 italic flex-grow text-center leading-relaxed mb-5" style={{ fontFamily: "'Nunito', sans-serif" }}>
            &ldquo;{testimonial.content}&rdquo;
          </p>
          <div className="w-full h-px mb-4" style={{ background: "linear-gradient(90deg, transparent, #fcebdc, transparent)" }} />
          <div className="flex items-center gap-3">
            <div className="relative flex-shrink-0">
              <img src={testimonial.avatar} alt={testimonial.author} className="w-10 h-10 rounded-full object-cover transition-all duration-300 group-hover:ring-2 group-hover:ring-[#E6792A]/30" style={{ border: "2px solid #fcebdc" }} loading="lazy" />
              <div className="absolute -bottom-0.5 -right-0.5 w-3 h-3 bg-green-400 rounded-full border-2 border-white" />
            </div>
            <div className="flex flex-col">
              <span className="text-[14px] font-bold text-gray-900 leading-tight" style={{ fontFamily: "'Nunito', sans-serif" }}>{testimonial.author}</span>
              <span className="text-[12px] text-gray-500 italic" style={{ fontFamily: "'Nunito', sans-serif" }}>{testimonial.role}</span>
            </div>
          </div>
        </div>
      </motion.div>
