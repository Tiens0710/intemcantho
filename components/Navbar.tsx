...
export default function Navbar() {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 right-0 z-50 w-full transition-[background-color,box-shadow,backdrop-filter] duration-300 ${shouldUseGlassHeader
      ...
      <div className="container mx-auto px-4">
        <div className="grid grid-cols-[auto_1fr_auto] items-center h-20 gap-6">
              ...
              >
                {item.megaMenu ? (
                  ...
                  >
                    <Link
                      href={item.href}
                      className={`group relative inline-flex items-center gap-1 px-4 py-2 text-[15px] font-bold tracking-wide transition-colors ${shouldUseGlassHeader
                    ...
                    <AnimatePresence>
                      {activeMegaMenu === item.label && (
                        <motion.div
                          ...
                          style={{ perspective: "1500px", transformOrigin: "top center" }}
                          className="fixed left-1/2 top-[84px] z-50 w-[min(1400px,calc(100vw-3rem))] -translate-x-1/2 overflow-hidden rounded-[42px] bg-white p-[2px] shadow-[0_50px_140px_-30px_rgba(15,23,42,0.6),0_0_1px_rgba(0,0,0,0.2)]"
                        >
                          {/* specula highlight border */}
                  ...
                  <Link
                    href={item.href}
                    className={`group relative inline-flex items-center px-4 py-2 text-[15px] font-bold tracking-wide transition-colors ${shouldUseGlassHeader
                      ? "text-black/90 hover:text-black"
                      : "text-white/95 hover:text-white"
                      }`}
                  >
          ...
          <div className="hidden md:flex gap-4 items-center justify-self-end">
            {persona && (
              <motion.div
                ...
                className={`px-3 py-1 rounded-full text-[11px] font-bold tracking-tighter border shadow-sm ${shouldUseGlassHeader
            ...
            <Link
              href="/dang-nhap"
              className={`px-6 py-2.5 text-sm font-bold tracking-tight transition-all rounded-full ${shouldUseGlassHeader
          ...
          <button
            ...
            className={`md:hidden p-2.5 rounded-xl transition-all ${shouldUseGlassHeader
        ...
        <AnimatePresence>
          {isOpen && (
            <motion.div
              ...
              className={`md:hidden overflow-hidden rounded-3xl mb-4 border transition-all ${shouldUseGlassHeader
                ...
                {navigationData.map((item) => (
                  <div key={item.label} className="space-y-1">
                    <div className="flex items-center justify-between">
                      <Link
                        href={item.href}
                        className={`flex-1 px-4 py-3 text-base font-bold rounded-2xl transition-all ${shouldUseGlassHeader
                      ...
                      {item.megaMenu && (
                        <button
                          ...
                          className={`p-3 rounded-2xl transition-all ${shouldUseGlassHeader ? "text-amber-800 hover:bg-amber-50" : "text-white hover:bg-white/10"
                            }`}
                        >
                            ...
                            <div className="grid grid-cols-1 gap-1">
                              {column.items.map(subItem => (
                                <Link
                                  key={subItem.label}
                                  href={subItem.href}
                                  className={`px-3 py-2 text-sm font-medium rounded-xl transition-all ${shouldUseGlassHeader ? "text-slate-600 hover:bg-amber-50 hover:text-amber-900" : "text-gray-300 hover:bg-white/5 hover:text-white"
                                    }`}
